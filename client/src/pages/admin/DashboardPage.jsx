...
const DashboardPage = () => {
  ...
  const kpis = [
    { label: 'Total Revenue', value: formatPrice(orderStats?.totalRevenue || 0), icon: FiTrendingUp, color: 'var(--primary-color)', bg: 'var(--bg-sub)', trend: '+12.5%' },
    { label: 'Total Orders', value: orderStats?.totalOrders || 0, icon: FiShoppingBag, color: 'var(--primary-color)', bg: 'var(--bg-sub)', trend: '+8.2%' },
    { label: 'Total Customers', value: stats?.totalUsers || 0, icon: FiUsers, color: 'var(--primary-color)', bg: 'var(--bg-sub)', trend: '+2.4%' },
    { label: 'Delivered Today', value: orderStats?.deliveredToday || 0, icon: FiTruck, color: 'var(--success-color)', bg: 'var(--bg-sub)', trend: 'Live' },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', paddingBottom: '2.5rem', borderBottom: '1px solid var(--border-color)' }}>
        <div>
           <p style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--accent-color)', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: '0.5rem' }}>Analytics Overview</p>
           <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.5rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.02em' }}>Dashboard</h1>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem 1.5rem', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
           ...
           <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-main)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>System Online</span>
        </div>
      </div>
      ...
      {orderStats?.statusCounts && (
        <div style={{ background: 'var(--bg-card)', borderRadius: '24px', border: '1px solid var(--border-color)', padding: '2rem', boxShadow: '0 10px 30px rgba(0,0,0,0.02)' }}>
          <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.25rem', fontWeight: 900, color: 'var(--text-main)', marginBottom: '1.5rem' }}>Order Status Breakdown</h3>
            ...
            {orderStats.statusCounts.map((sc, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <p style={{ fontSize: '0.65rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '4px' }}>{sc._id?.replace(/_/g, ' ') || 'Unknown'}</p>
                </div>
              );
            })}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '3rem' }}>
        ...
        <div style={{ flex: 2 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.25rem', fontWeight: 900, color: 'var(--text-main)' }}>Recent Orders</h3>
           </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                 <thead>
                    <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                       {['Order / Tracking', 'Customer', 'Amount', 'Status'].map(h => (
                             ...
                             <span style={{
                               ...
                               background: 'var(--bg-sub)',
                                ...
                                border: '1px solid var(--border-color)'
                                 ...
                                 {order.status?.replace(/_/g, ' ') || (order.isPaid ? 'Paid' : 'Pending')}
                              </span>
                          </td>
                       </tr>
                    ))}
             ...
             display: 'flex', flexDirection: 'column', gap: '1rem'
              ...
              {recentFeedback.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)' }}>No feedback yet</p>
                  <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>Feedback will appear here once customers rate their deliveries</p>
                </div>
              ) : (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1rem', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-color)', width: 'fit-content', margin: '0 auto' }}>
         ...
         <p style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Data updated in real-time</p>
      </div>
    </div>
  );
};
