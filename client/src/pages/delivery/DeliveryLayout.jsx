...
import { FiTruck, FiHome, FiCheckSquare, FiLogOut, FiPackage, FiSun, FiMoon } from 'react-icons/fi';
...
const DeliveryLayout = () => {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-main)', color: 'var(--text-main)', transition: 'background 0.3s ease' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '8px 16px', marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h2 style={{ fontSize: '1.1rem', fontWeight: '700', margin: 0 }}>Delivery</h2>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', margin: 0 }}>Driver Portal</p>
            </div>
          </div>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
          <NavLink to="/delivery" end style={({ isActive }) => linkStyle(isActive)}>
            <FiHome size={18} /> Dashboard
          </NavLink>
          <NavLink to="/delivery/complete" style={({ isActive }) => linkStyle(isActive)}>
            <FiCheckSquare size={18} /> Complete Order
          </NavLink>
        </nav>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div style={{ overflow: 'hidden' }}>
              <p style={{ fontWeight: '600', fontSize: '0.9rem', margin: 0, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{user?.name || 'Driver'}</p>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', margin: 0, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{user?.email || ''}</p>
            </div>
          </div>
      ...
      {/* Main */}
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto', background: 'var(--bg-main)' }}>
